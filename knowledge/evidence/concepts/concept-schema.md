...
Every logical concept record must include exactly these eight required fields:
...
`knowledge/evidence/concepts/concepts_curated.csv` is the canonical analytical implementation of the concept schema inside Atlas.

It preserves the eight logical fields while also storing:

- lineage columns such as `Concept ID`, `Track`, `Batch`, `Raw Source ID`, and `Original Wording`
- curation columns such as `Notes` and `Why Now`
- a normalized implementation of the logical `Job` field as:
  - `Canonical Job`
  - `Domain`

This separation is intentional. It lets Atlas compare similar outcomes across different operating contexts without mutating the original concept.
...
In the analytical CSV, this logical field is implemented as `Canonical Job` plus `Domain`.
...
3. `Primitive`, logical `Job`, `Customer`, `Value Mechanism`, and `Initial Wedge` must be specific enough that two reviewers could compare concepts side by side without reinterpretation.
...
- If Atlas learns that the schema itself should change, record that decision in `core/DECISION_LOG.md` and create a new schema version rather than mutating `v1.0`.
